        ...
        <div class="col-lg-12">
            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-title">
                    ...
                    <h5 class="shop_title">Monthly Reports </h5>
...
<canvas id="salesChart" width="800" height="200" style="background-color: #fff;"></canvas>
                <div class="ibox-content">
                    ...
                    <form method="get" action="<?php echo base_url(); ?>vendors/order_monthly_reports">
                        <div class="form-group row">
                            <div class="col-sm-4"><label for="invocieno" class="caption">From Date</label>
                                <div class="input-group">
                                    <div class="input-group-addon"><span class="icon-file-text-o" aria-hidden="true"></span></div>
                                    <input value="<?= $filter_from_dt != "" ? $filter_from_dt : "" ?>" type="date"  class="form-control" name="from_date" required onchange="setTodate(this)">
                                </div>
                            </div>
                            <div class="col-sm-4"><label for="invocieno" class="caption">To Date </label>
                                <div class="input-group">
                                    <div class="input-group-addon"><span class="icon-bookmark-o" aria-hidden="true"></span></div>
                                    <input value="<?= $filter_to_dt != "" ? $filter_to_dt : "" ?>" type="date"  class="form-control" name="to_date" id="to_date" required>
                                </div>
                            </div>
                            <div class="col-sm-4"><label for="invocieno" class="caption"></label>
                                <div class="input-group">
                                    <input type="submit" class="btn btn-success mt-3" value="Filter">&nbsp;&nbsp;
                                    <a href="<?= base_url() ?>vendors/order_monthly_reports"><input type="button" class="btn btn-danger mt-3" name="reset" value="Reset"></a>
                                </div>
                            </div>
                        </div>
                    </form>
                    <!-- <div class="row">
                   <form method="post" class="form-horizontal" enctype="multipart/form-data" action="<?php echo base_url(); ?>vendors/order_monthly_reports/datewiseReport">
                    ...
                    ?>" required=""></div><div class="col-md-2 action" style="bottom:-22px">
                        <input type="submit" class="btn btn-primary" value="GET"></div></br>
                    </form>
                </div></br>
                </hr> -->
                                ...
                                <tr class="gradeX">
                                    ...
                                    <td style="width: 100%; height:100%;">
                                        <table class="table table-striped table-bordered table-hover">
                                            ...
                                            <?php
                                            $vendor_qry=$this->db->query("select * from vendor_shop where id='".$value->vendor_id."'");
                                            $vendor_row=$vendor_qry->row();
                                            $cart_qry = $this->db->query("select * from cart where session_id='" . $value->session_id . "' and vendor_id='" . $value->vendor_id . "'");
     ...
     foreach ($jsonatt as $attribute) {
         ...
         if ($title_query && $value_query) {
             ...
             echo ucfirst($title_res->title) . ': ' . ucfirst($value_res->value);
                                                    ...
                                                    <td><?= $scat_row->sub_category_name ? $scat_row->sub_category_name : 'N/A' ?></td>
                                            ...
                                            ?>
                                                    <td><?php echo $admin_comission . "%"; ?>
                                                    (<?php 
                                            echo (($admin_comission* $cart_value->unit_price)/100);?>
                                            )</td>
                                            <td>
                                        <?php 
                                        echo ($adminc_row->gst);?>%
                                ...
                                echo ($commision); ?>
                                    ...
                                    <!-- <td><?php echo $value->vendor_commission; ?></td> -->
                                    <td><?php 
...
if (total_sales !== 0) {
    var formatted_total_sales = "$ " + total_sales + "<br> Total Monthly sales";
