...
<canvas id="salesChart" width="800" height="200" style="background-color: #fff;"></canvas>
                <div class="ibox-content">
                        <div class="row">
                    <form method="post" class="form-horizontal" enctype="multipart/form-data" action="<?php echo base_url(); ?>admin/order_monthly_reports/datewiseReport">
                       <div class="col-md-2">
                        <label>From Date</label>
                        <input type="date" class="form-control" name="start_date" value="<?php if($start_date!=''){ echo $start_date; }?>" required="" onchange="setTodate(this)">
                      </div>
                       <div class="col-md-2">
                        <label>To Date</label>
                        <input type="date" class="form-control" name="end_date" id="end_date" value="<?php if($end_date!=''){ echo $end_date; }?>" required="">
                       </div>
                          <div class="col-md-2" style="bottom:-22px">
                        <input type="submit" class="btn btn-primary" value="GET">
                        <a href="<?php echo base_url(); ?>admin/order_monthly_reports" class="btn btn-warning">RESET</a>
                    </div>
                    </form>
                </div>
                <hr/>
                            ...
                            foreach ($orders_commission as $value) {
                               ...
                               <td style="width: 100%; height:100%;">
                                    <table class="table table-striped table-bordered table-hover">
                                        ...
                                        <?php $vendor_qry=$this->db->query("select * from vendor_shop where id='".$shop->id."'");
    ...
    options: {
        scales: {
            y: {  // Changed from 'yAxes' to 'y' for Chart.js 3.x
                beginAtZero: true,
                ticks: {
                    stepSize: 1,  // Ensure ticks at integer intervals
                    callback: function(value) {  // This callback forces the tick to be an integer
                        if (value % 1 === 0) {
                            return value;
                        }
                    }
        ...
        plugins: {
            tooltip: {  // Changed from 'tooltips' to 'tooltip' in Chart.js 3.x
                mode: 'index',
                intersect: false,
                callbacks: {
