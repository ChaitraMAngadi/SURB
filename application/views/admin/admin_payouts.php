    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="ibox float-e-margins">
                ...
                <div class="row">
                   <form method="post" class="form-horizontal" enctype="multipart/form-data" action="<?php echo base_url(); ?>admin/admin_payouts/datewiseReport">
                   <div class="col-md-2">
                   <label>From Date :</label>
                    ...
                    <a href="<?php echo base_url(); ?>admin/admin_payouts" class="btn btn-warning">RESET</a>
                    </div>
                    </form></div><hr/>
                    ...
                    <div class="table-responsive">
                        ...
                        <table class="table table-striped table-bordered table-hover dataTables-example" id="data-table">
                            ...
                            <tbody>
                                <?php
                                // foreach ($orders as $v) {
                                //     $vendor_qry=$this->db->query("select * from vendor_shop where id='".$v->vendor_id."'");
         ...
         if ($title_query && $value_query) {
             ...
             echo ucfirst($title_res->title) . ': ' . ucfirst($value_res->value);
                                      ...
                                      <?php }?>
                                       ...
                                       $chk_vend = $this->db->query("select * from vendor_payements where vendor_id='" . $v->vendor_id . "'");
                                       $chk_vend_row= $chk_vend->row();
                                        ...
                                        ?></td>
                                            ...
                                            <td>
                                                ...
                                                if($payment_qry_res->status == 1){?>
                                                       ...
                                                       <a href="<?php echo base_url(); ?>admin/Admin_payouts/getBackAmount/<?php echo $v->vendor_id; ?>/<?php echo $v->id;?>">
...
if($payment_qry_res->reference_id == ''||$payment_qry_res->payment_status=='pending'|| $payment_qry_res->payment_status=='deferred') {
?>
<form method="post" class="form-horizontal" enctype="multipart/form-data"  action="<?= base_url() ?>admin/admin_payouts/changeStatus">
    <input type="hidden" name="id" value="<?php echo $vendor_row->id; ?>">
    <input type="hidden" name="reference_id" value="<?php echo $v->id;?>">
    <input type="hidden" id="requested_amount"  name="requested_amount" class="form-control" value="<?php echo $vendorAmount; ?>">
    <input type="hidden" value="offline" name="mode_payment"/>
    <input type="hidden" id="sender_name" name="sender_name" class="form-control" value="admin"/>
    <input type="hidden" id="receiver_name" name="receiver_name" class="form-control" value="<?php echo $vendor_row->owner_name;?>">
    <input type="hidden" id="description" name="description" class="form-control" value="paid">
    <div><div>
    <select name="status" class="form-control">
    <option value="pending" <?php if ($payment_qry_res->payment_status == 'pending') echo 'selected'; ?>>Pending</option>
    <option value="completed" <?php if ($payment_qry_res->payment_status == 'completed') echo 'selected'; ?>>Completed</option>
    <option value="deferred" <?php if ($payment_qry_res->payment_status == 'deferred') echo 'selected'; ?>>Deferred</option>
</select>
...
<script>
    ...
    printWindow.document.write('<html><head><title>Admin Payout Data</title></head><body>');
    printWindow.document.write('<table border="1">');
    printWindow.document.write('<tr><td colspan="' + row.cells.length + '">Payout Data:</td></tr>');
