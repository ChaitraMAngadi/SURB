...
<script>
//    set last notification id in session
    setInterval(function () {
        $.ajax({
            url: "<?= base_url() ?>admin/notifications/new_notification",
            type: "post",
            success: function (data) {
                if (data == 1) {
                    const audio = new Audio("<?= base_url() ?>admin_assets/music/notification.mp3");
                    audio.play();
                } else {
