...
class Products extends MY_Controller {
    ...
    function index() {
        ...
        $count = $this->db->query("select * from products where status=1 order by id desc");
        $this->data['count_data'] = $count->num_rows();

        $config['base_url'] = base_url() . 'vendors/products';
...
//        $this->data['products'] = $qry->result();
        ...
        if ($keyword) {
            $this->db->where("(p.id LIKE '%" . $keyword . "%' OR p.name LIKE '%" . $keyword . "%' OR c.category_name LIKE '%" . $keyword . "%' OR sub.sub_category_name LIKE '%" . $keyword . "%')");
        }
    ...
    function stockManagement($pid, $vid) {
        $this->data['pid'] = $pid;
        $this->data['vid'] = $vid;
        $qry = $this->db->query("select * from stock_management where varient_id='" . $vid . "' and product_id='" . $pid . "' order by id desc");
        $this->data['stock'] = $qry->result();

        //redirect(''.base_url().'vendors/products/stock_management/'.$pid."/".$vid);
    ...
    function add_product() {
        ...
        $qry = $this->db->query("select * from admin_comissions where status=1 and shop_id='" . $shop_id . "' group by cat_id");
        $resul = $qry->result();
        $ar = [];
        foreach ($resul as $value) {
            ...
            $ques = $this->db->query("select *from questionaries where cat_id='" . $category->id . "'");
            $question = $ques->result();
            //echo $this->db->last_query(); die;
            $option = $this->db->query("select * from options where ques_id='" . $question->id . "'");
            $options = $option->row();

            $ar[] = array('id' => $category->id, 'category_name' => $category->category_name, 'questions' => $question->question, 'option' => $options->option);
        }
...
//        $this->data['questionary'] = $data;
    ...
    //         $insert_filter_query = $this->db->insert('product_filter', $filter_ar);
    //     }
    //     if ($insert_query) {
    ...
    function insert_product() {
        ...
        $brand = ($this->input->get_post('brand') != '') ? $this->sanitize($this->input->get_post('brand')) : 0;
    
        $producttags = ($this->input->get_post('product_tags') != '') ? implode(",", array_map([$this, 'sanitize'], $this->input->get_post('product_tags'))) : '';
        ...
        $ar = array(
            ...
            'option_id' => implode(",", array_map([$this, 'sanitize'], $this->input->get_post('option_id'))),
        ...
        if ($insert_query) {
            ...
            if ($variant_product == 'no') {
                ...
                $vid = $this->db->insert_id($ins);
                foreach ($productimage as $image) {
                    $ar = array('product_id' => $last_id, 'variant_id' => $vid, 'image' => $this->sanitize($image));
                    $this->db->insert("product_images", $ar);
                }
...
//               die;
        ...
        if ($insert_query) {
            ...
            $this->session->set_tempdata('error_message', 'Unable to add',3);
            redirect('vendors/products/edit_product');
            die();
        }
    }
    ...
    private function upload_multiplefile($file_name) {
        ...
        foreach ($files['name'] as $key => $image) {
            ...
            $fileName = date("YmdHis") . rand(0, 9999999) . ".png";
...
//                    $this->db->insert("stock_management", $stock_ar);
                    $this->db->where('id', $pid)->update('products', ['status' => 0, 'updated_at' => time()]);

                    //notification to admin
                    $msg = "Product Inactive: Pricing changed for Product ID: #" . $pid;
...
//                    $this->db->insert("stock_management", $stock_ar);
    ...
    function deleteProductImageInEdit($id, $pid, $vid) {
        ...
        $del = $this->db->delete("product_images", array('id' => $img_id));
        if ($del) {
            redirect('vendors/products/edit/' . $pid);
        }
    }
    ...
    function uploadImages() {
        ...
        if ($image != '') {
            ...
            $ar = array('product_id' => $pid, 'variant_id' => $vid, 'image' => $image, 'priority' => $priority);
            $ins = $this->db->insert("product_images", $ar);
...
// die;
    ...
    function loadQuestionary() {
        ...
        $chk = $this->vendor_model->getQuestion($cid, $subcatid);
        die;
    }

    function loadOptions() {
        $quesid = $this->input->get_post('quesid');

        $chk = $this->vendor_model->getOption($quesid);
        die;
    }
    ...
    function loadFilters() {
        $cid = $this->input->get_post('cid');
        $subcatid = $this->input->get_post('subcatid');
        $chk = $this->vendor_model->getFilters($cid, $subcatid);
        die;
    }

    function loadFilterOption() {
        $filter_id = $this->input->get_post('filter_id');
        $chk = $this->vendor_model->getFilterOption($filter_id);
        die;
    }
    ...
    function linkvariant($pid) {
        ...
        foreach ($varint as $value) {
            /* $qry = $this->db->query("select * from link_variant_attributes where link_variant_id='".$value->id."'");
              ...
              $attributes[]=array('id'=>$vFalue1->id,'type'=>$types->title,'value'=>$values->value);
              } */
    ...
    function show_filters($id) {

        $this->data['filters'] = $this->vendor_model->get_view_filters($id);

        $this->load->view('vendors/includes/header', $this->data);
        $this->load->view('vendors/view_filter_options', $this->data);
        $this->load->view('vendors/includes/footer');
    }
    ...
    function loadbrands() {
        ...
        $chk = $this->vendor_model->getbrands($subcatid);
        //print_r($chk);die;
        if ($chk) {
            redirect('vendors/products');
        }
