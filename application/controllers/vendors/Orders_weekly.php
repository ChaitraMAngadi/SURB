...
class Orders_weekly extends MY_Controller {
    ...
    function index() {
        ...
        if ($type == "today_orders") {
            $data['count_data'] = sizeof($this->vendor_model->get_today_orders($data['shop_id']));
            $config['base_url'] = base_url() . 'vendors/orders_weekly';
            ...
            $all_orders = $this->vendor_model->get_today_orders($data['shop_id']);
            ...
            $where = array('vendor_id' => $data['shop_id'],
            'payment_status' => 1,
            'order_date >=' => $start_date,
            'order_date <=' => $end_date
        );
            $all_order = $this->common_model->get_data_with_condition($where , 'orders');
            ...
            $all_orders = $this->common_model->get_data_with_condition($where, 'orders');
            // print_r($this->db->last_query());die;
        }
        // echo "<pre>";
        ...
        $this->load->view('vendors/orders_weekly', $this->data);
    ...
    // exit;
        ...
        // redirect('vendors/orders_weekly');
    }
...
if (isset($res['packages']) && is_array($res['packages']) && !empty($res['packages'])) {
    ...
    foreach ($res['packages'] as $package) {
        ...
        if (isset($package['pdf_download_link'])) {
            // $this->load->view('vendors/orders_weekly');
            echo '<div>';
            echo '<button class="btn btn btn-success"><a href="' . base_url() . 'vendors/orders_weekly/">Back button</a></button>';
...
<html lang="en">
    ...
    <body>
        ...
        if ($this->email->send()) {
            ...
            redirect('vendors/orders_weekly/return');
        } else {
            $this->session->set_tempdata('error_message', 'Something Went Wrong.',3);
            redirect('vendors/orders_weekly/return');
        }
