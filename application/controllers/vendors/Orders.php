...
class Orders extends MY_Controller {
    ...
    function index() {
        ...
        if ($type == "today_orders") {
            ...
            $config['base_url'] = base_url() . 'vendors/orders';
            ...
            $all_order = $this->common_model->get_data_with_condition(['vendor_id' => $data['shop_id'],'payment_status' => 1] , 'orders');
            ...
            $all_orders = $this->common_model->get_data_with_condition(['vendor_id' => $data['shop_id'],'payment_status' => 1], 'orders');
            // print_r($this->db->last_query());die;
        }
        // echo "<pre>";
...
if (isset($res['packages']) && is_array($res['packages']) && !empty($res['packages'])) {
    ...
    foreach ($res['packages'] as $package) {
        ...
        if (isset($package['pdf_download_link'])) {
            ...
            echo '<div>';
            echo '<button class="btn btn btn-success"><a href="' . base_url() . 'vendors/orders/">Back button</a></button>';
...
<html lang="en">
    ...
    <body>
        ...
        if ($this->email->send()) {
            ...
            redirect('vendors/orders/return');
        } else {
            $this->session->set_tempdata('error_message', 'Something Went Wrong.',3);
            redirect('vendors/orders/return');
        }
