...
class Orders_monthly extends MY_Controller {
    ...
    function index() {
        ...
        if ($type == "today_orders") {
            ...
            $config['base_url'] = base_url() . 'vendors/orders_monthly';
            ...
            $where = array('vendor_id' => $data['shop_id'],'payment_status' => 1,
        ...
        $this->load->view('vendors/orders_monthly', $this->data);
    ...
    // exit;
        ...
        // redirect('vendors/orders_monthly');
    }
...
if (isset($res['packages']) && is_array($res['packages']) && !empty($res['packages'])) {
    ...
    foreach ($res['packages'] as $package) {
        ...
        if (isset($package['pdf_download_link'])) {
            // $this->load->view('vendors/orders_monthly');
            echo '<div>';
            echo '<button class="btn btn btn-success"><a href="' . base_url() . 'vendors/orders_monthly/">Back button</a></button>';
...
<html lang="en">
    ...
    <body>
        ...
        if ($this->email->send()) {
            ...
            redirect('vendors/orders_monthly/return');
        } else {
            $this->session->set_tempdata('error_message', 'Something Went Wrong.',3);
            redirect('vendors/orders_monthly/return');
        }
