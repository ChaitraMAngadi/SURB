...
class Orders_daily extends MY_Controller {
    ...
    function index() {
        ...
        $config['base_url'] = base_url() . 'admin/Orders_daily';
        ...
        $where = array('order_status !=' => 1,'order_date'=>$current_date);
        $table = "orders";
        $this->db->select("*");
        $this->db->order_by('id', 'desc');
...
//        $data['orders'] = $qry->result();
        ...
        $this->load->view('admin/orders_daily', $this->data);
    ...
    function pending($value) {
        ...
        if ($value == 'pending') {
            $count = $this->db->query("select * from orders where order_status=1 group by session_id order by id desc");
            $this->data['count_data'] = $count->num_rows();

            $config['base_url'] = base_url() . 'admin/Orders_daily/pending/pending';
            $config['total_rows'] = $this->data['count_data'] = $count->num_rows();
            $config['per_page'] = 10;
            // $this->data['page_title'] = 'Total '.$config['total_rows'].' Books';
            $config['page_query_string'] = true;
...
//            $qry = $this->db->query("select * from orders where order_status=1 order by id desc");
            ...
            $qry = $this->db->query("select * from orders where order_status=2 and accept_status=0 and order_date ='$current_date' group by session_id order by id desc");
        } else if ($value == 'accepted_vendor') {

            $count = $this->db->query("select * from orders where order_status=2  and order_date ='".$current_date."' group by session_id order by id desc");
            $this->data['count_data'] = $count->num_rows();

            $config['base_url'] = base_url() . 'admin/Orders_daily/pending/accepted_vendor';
            $config['total_rows'] = $this->data['count_data'] = $count->num_rows();
            $config['per_page'] = 10;
            // $this->data['page_title'] = 'Total '.$config['total_rows'].' Books';
            $config['page_query_string'] = true;
            ...
            $where = array('order_status' => 2,'order_date'=>$current_date);
            $table = "orders";
            $this->db->select("*");
            $this->db->order_by('id', 'desc');
            $qry = $this->db->where($where)->group_by('session_id')->get($table);
...
//            $qry = $this->db->query("select * from orders where order_status=2 order by id desc");
            ...
            $count = $this->db->query("select * from orders where order_status=3 and order_date='".$current_date."' group by session_id order by id desc");
            $this->data['count_data'] = $count->num_rows();

            $config['base_url'] = base_url() . 'admin/Orders_daily/pending/pickuorders';
            $config['total_rows'] = $this->data['count_data'] = $count->num_rows();
            $config['per_page'] = 10;
            // $this->data['page_title'] = 'Total '.$config['total_rows'].' Books';
            $config['page_query_string'] = true;
            ...
            $where = array('order_status' => 3,'order_date'=>$current_date);
            $table = "orders";
            $this->db->select("*");
            $this->db->order_by('id', 'desc');
            $qry = $this->db->where($where)->group_by('session_id')->get($table);
...
//            $qry = $this->db->query("select * from orders where order_status=3 order by id desc");
            ...
            $count = $this->db->query("select * from orders where order_status=4 and order_date='".$current_date."' group by session_id order by id desc");
            $this->data['count_data'] = $count->num_rows();

            $config['base_url'] = base_url() . 'admin/Orders_daily/pending/ondelivery';
            $config['total_rows'] = $this->data['count_data'] = $count->num_rows();
            $config['per_page'] = 10;
            // $this->data['page_title'] = 'Total '.$config['total_rows'].' Books';
            $config['page_query_string'] = true;
            ...
            $where = array('order_status' => 4,'order_date'=>$current_date);
            $table = "orders";
            $this->db->select("*");
            $this->db->order_by('id', 'desc');
            $qry = $this->db->where($where)->group_by('session_id')->get($table);
...
//            $qry = $this->db->query("select * from orders where order_status=4 order by id desc");
            ...
            $count = $this->db->query("select * from orders where order_status=5 and order_date='".$current_date."' group by session_id order by id desc");
            $this->data['count_data'] = $count->num_rows();

            $config['base_url'] = base_url() . 'admin/Orders_daily/pending/completed';
            $config['total_rows'] = $this->data['count_data'] = $count->num_rows();
            $config['per_page'] = 10;
            // $this->data['page_title'] = 'Total '.$config['total_rows'].' Books';
            $config['page_query_string'] = true;
            ...
            $where = array('order_status' => 5,'order_date'=>$current_date);
            $table = "orders";
            $this->db->select("*");
            $this->db->order_by('id', 'desc');
            $qry = $this->db->where($where)->group_by('session_id')->get($table);
...
//            $qry = $this->db->query("select * from orders where order_status=5 order by id desc");
            ...
            $count = $this->db->query("select * from orders where order_status=6 and order_date='".$current_date."' group by session_id order by id desc");
            $this->data['count_data'] = $count->num_rows();

            $config['base_url'] = base_url() . 'admin/Orders_daily/pending/cancelled';
            $config['total_rows'] = $this->data['count_data'] = $count->num_rows();
            $config['per_page'] = 10;
            // $this->data['page_title'] = 'Total '.$config['total_rows'].' Books';
            $config['page_query_string'] = true;
            ...
            $where = array('order_status' => 6,'order_date'=>$current_date);
            $table = "orders";
            $this->db->select("*");
            $this->db->order_by('id', 'desc');
            $qry = $this->db->where($where)->group_by('session_id')->get($table);
...
//            $qry = $this->db->query("select * from orders where order_status=6 order by id desc");
            ...
            $count = $this->db->query("select * from orders where order_status=7 and order_date='".$current_date."' group by session_id order by id desc");
            $this->data['count_data'] = $count->num_rows();

            $config['base_url'] = base_url() . 'admin/Orders_daily/pending/return';
            $config['total_rows'] = $this->data['count_data'] = $count->num_rows();
            $config['per_page'] = 10;
            // $this->data['page_title'] = 'Total '.$config['total_rows'].' Books';
            $config['page_query_string'] = true;
            ...
            $where = array('order_status' => 7,'order_date'=>$current_date);
            $table = "orders";
            $this->db->select("*");
            $this->db->order_by('id', 'desc');
            $qry = $this->db->where($where)->group_by('session_id')->get($table);
...
//            $qry = $this->db->query("select * from orders where order_status=7 order by id desc");
            ...
            $count = $this->db->query("select * from orders where order_date='".$current_date."' group by session_id order by id desc");
            $this->data['count_data'] = $count->num_rows();

            $config['base_url'] = base_url() . 'admin/Orders_daily/pending/allorders';
            $config['total_rows'] = $this->data['count_data'] = $count->num_rows();
            $config['per_page'] = 10;
            // $this->data['page_title'] = 'Total '.$config['total_rows'].' Books';
            $config['page_query_string'] = true;
...
//            $qry = $this->db->query("select * from orders order by id desc");
        ...
        foreach ($orders_get as $row) {
            $vendor_orders = $this->common_model->get_data_with_condition(['session_id' => $row->session_id,'order_date'=>$current_date], 'orders');
            //get all status
    ...
    function assigned_deliveryBoy($order_id) {
        ...
        if ($update_status) {
            ...
            $otp_message = "Dear " . $order_details['ordersdetails']['customer_name'] . ", your order no #" . $order_details['ordersdetails']['id'] . " is assigned to delivery-person and he/she will deliver to you shortly. Thank you for shopping with us. Thanks and Regards Absolutemens.com Fashion services Pvt.Ltd";
...
Absolutemens.com";
    ...
    function assignDeliveryBoy() {
        ...
        if ($upd) {
            ...
            $otp_message = "Dear " . $user_row->first_name . " your order number " . $this->input->get_post('order_id') . " is assigned to " . $delivery_row->name . " and he/she will deliver to you shortly. Thank u for shopping with us.";
