...
class Products extends MY_Controller {
    ...
    function index() {
        ...
        $count = $this->db->where('status', '1')->get('products');
        ...
        $this->db->select('products.*,products.id as pid,products.created_at as created,products.updated_at as updated,products.status as prod_status,products.cat_id as p_cat_id,categories.*,sub_categories.*');
        $this->db->from('products');
        $this->db->join('categories', 'products.cat_id = categories.id', 'left');
        $this->db->join('sub_categories', 'products.sub_cat_id = sub_categories.id', 'left');
        $this->db->where('products.status', '1');
        if ($category != "") {
            $this->db->where('products.cat_id', $category);
        }
        if ($keyword != "") {
            $this->db->like('products.name' ,$keyword);
            $this->db->or_like('products.id', $keyword);
            $this->db->or_like('categories.category_name', $keyword);
            $this->db->or_like('sub_categories.sub_category_name', $keyword);
        }
...
//        $qry = $this->db->get("select * from products where status=1 order by id desc");
//        $this->data['products'] = $qry->result();
    ...
    function product_images($pid, $vid) {
        ...
        $this->data['vid'] = $vid;
        $this->load->view('admin/includes/header', $this->data);
        $this->load->view('admin/product_images', $this->data);
        $this->load->view('admin/includes/footer');
    }
    ...
    function stockManagement($pid, $vid) {
        ...
        $this->load->view('admin/includes/header', $this->data);
        $this->load->view('admin/stock_management', $this->data);
        $this->load->view('admin/includes/footer');
    }
    ...
    function create_stock($pid, $vid) {
        ...
        $this->load->view('admin/includes/header', $this->data);
        $this->load->view('admin/add_stock', $this->data);
        $this->load->view('admin/includes/footer');
    }
    ...
    function insertStock() {
        ...
        if ($ins) {
            ...
            redirect(base_url() . 'admin/products/stockManagement/' . $pid . "/" . $vid);
    ...
    function childproducts() {
        ...
        $this->load->view('admin/includes/header', $this->data);
        $this->load->view('admin/child_products', $this->data);
        $this->load->view('admin/includes/footer');
    }
...
//        $this->data['subcategories'] = $this->db->get('sub_categories')->result();
        if($products->sub_cat_id == '') {
        $this->data['questionaries'] = $this->db->where(array("cat_id" => $products->cat_id))->get('questionaries')->result();
        } else {
        $this->data['questionaries'] = $this->db->where(array("sub_cat_id" => $products->sub_cat_id))->get('questionaries')->result();    
        }
        ...
        $this->data['filters'] = $this->db->where(array("cat_id" => $products->cat_id))->get('filters')->result();
            ...
            $this->db->where("find_in_set($products->sub_cat_id, sub_cat_id)");
    ...
    function product_statatics($pid) {
        $this->data['product_id'] = $pid;

//        $result = $this->admin_model->get_product_id($pid);
//        $this->data['result']=$result;

        $product = $this->db->query("select * from products where id='" . $pid . "'");
        $this->data['products'] = $product->row();

        $users = $this->admin_model->UsersData();
        $this->data['users'] = $users;

        $this->data['link_variant'] = $this->admin_model->get_view_data_row("link_variant", array("product_id" =>$this->data['products']->id));

        $this->load->view('admin/includes/header', $this->data);
        $this->load->view('admin/product_statatics', $this->data);
        $this->load->view('admin/includes/footer');
    }
    ...
    function addvariant($pid) {
        ...
        $this->load->view('admin/includes/header', $this->data);
        $this->load->view('admin/add_variant', $this->data);
        $this->load->view('admin/includes/footer');
    }
    ...
    function edit_variant($pid, $vid) {
        ...
        $this->load->view('admin/includes/header', $this->data);
        $this->load->view('admin/update_variant', $this->data);
        $this->load->view('admin/includes/footer');
    }
    ...
    function edit_link_variant($pid, $vid) {
        ...
        $this->load->view('admin/includes/header', $this->data);
        $this->load->view('admin/update_link_variant', $this->data);
        $this->load->view('admin/includes/footer');
    }
    ...
    function create_variant($pid) {
        ...
        $this->load->view('admin/includes/header', $this->data);
        $this->load->view('admin/create_variant', $this->data);
        $this->load->view('admin/includes/footer');
    }
    ...
    function update_variant() {
        ...
        redirect('admin/products/addvariant/' . $pid);
    ...
    function delete_link_variant($pid, $vid) {
        ...
        if ($del) {
            ...
            redirect('admin/products/linkvariant/' . $pid);
        }
    }
    ...
    function insert_product() {
        ...
        if ($insert_query) {
            ...
            if ($variant_product == 'no') {
                $insert = array('product_id' => $last_id, 'price' => $this->input->get_post('price'), 'saleprice' => $this->input->get_post('saleprice'), 'stock' => $this->input->get_post('stock'), 'status' => 1, 'created_at' => time());
                $ins = $this->db->insert('link_variant', $insert);
                $vid = $this->db->insert_id($ins);
                foreach ($productimage as $image) {
                    $ar = array('product_id' => $last_id, 'variant_id' => $vid, 'image' => $image);
    ...
    function update_product() {
        ...
        /* $tax_class = $this->input->get_post('tax_class');
          if($tax_class=='yes')
          {
        ...
        $data = array(
            ...
            //'taxname'=>$taxname,
            'brand' => $brand,
            'product_tags' => implode(",", $this->input->get_post('product_tags')),
            'meta_tag_title' => $this->input->get_post('meta_tag_title'),
            'meta_tag_description' => $this->input->get_post('meta_tag_description'),
            'meta_tag_keywords' => $this->input->get_post('meta_tag_keywords'),
            //'shop_id' => $_SESSION['vendors']['vendor_id'],
        ...
        if ($insert_query) {
            $ar = array('price' => $price, 'saleprice' => $saleprice, 'stock' => $stock);
            $this->db->update("link_variant", $ar, array('product_id' => $pid));
    ...
    /* function uploadImages()
      ...
      {
      $image=='';
      $this->session->set_tempdata('success_message', 'Product Image added Successfully');
      ...
      redirect('vendors/products/product_images/'.$pid."/".$vid);
    ...
    function insert_variant() {
        ...
        if ($chk->num_rows() > 0) {
            ...
            redirect(base_url() . 'vendors/products/addvariant/' . $this->input->get_post('product_id'));
        } else {
            $ar = array(
                'product_id' => $this->input->get_post('product_id'),
                'attribute_type' => $this->input->get_post('types'),
        ...
        if ($row_check->manage_stock == 'yes') {
            ...
            if ($chk->num_rows() > 0) {
                ...
                if ($ins) {
                    ...
                    redirect(base_url() . 'vendors/products/linkvariant/' . $pid);
                }
            }
    ...
    function insert_subproduct() {
        ...
        if ($insert_query) {
            ...
            $this->session->set_tempdata('success_message', 'Product Added Successfully',3);
            redirect('vendors/products/childproducts/' . $pid);
            die();
        } else {
            $this->session->set_tempdata('error_message', 'Unable to add',3);
            redirect('vendors/products/childproducts/' . $pid);
    ...
    function update_subproduct() {
        ...
        if ($insert_query) {
            ...
            redirect('vendors/products/childproducts/' . $old_pid);
            die();
        } else {
            $this->session->set_tempdata('error_message', 'Unable to add',3);
            redirect('vendors/products/childproducts/' . $old_pid);
    ...
    function delete_subproduct($opid, $pid) {
        if ($this->admin_model->delete_product($pid)) {
            ...
            redirect('vendors/products/childproducts/' . $opid);
            die();
        } else {
...
// die;
    ...
    function loadSubcategories() {
        ...
        $chk = $this->vendor_model->subcategories($cid, $shop_id, $type);
        die;
    }
    ...
    function create_link_variant($pid, $vid) {
        ...
        $this->load->view('admin/includes/header', $this->data);
        $this->load->view('admin/create_link_variant', $this->data);
        $this->load->view('admin/includes/footer');
    }
    ...
    function import_product() {
        ...
        $this->load->view('admin/includes/header', $this->data);
        $this->load->view('admin/import_product', $this->data);
        $this->load->view('admin/includes/footer');
    }

    function changeStatus($pid, $status) {
        $upd = $this->db->update("products", array('status' => $status), array('id' => $pid));
        if ($upd) {
            //create notification
            $vendor_id = $this->db->where('id', $pid)->get('products')->row()->shop_id;
            if ($status == 1) {
                $msg = "Product having ID #" . $pid . " activated by admin";
            } else if ($status == 0) {
                $msg = "Product having ID #" . $pid . " deactivated by admin";
            }

            $array = array('vendor_id' => $vendor_id, 'message' => $msg, 'status' => 0, 'created_date' => time());
    ...
    function importExcel() {
        ...
        if ($ext != 'csv') {
            ...
            $shop_id = $_SESSION['vendors']['vendor_id'];
            redirect(base_url() . 'vendors/products/import_product?shop_id=' . $shop_id);
        }
    ...
    function product_view($id) {
        ...
        $this->data['product_view'] = $this->admin_model->get_product_details($id);
        // print_r($this->data['product_view']->id); die;
        $this->data['varient_data'] = $this->admin_model->get_veriant_data($id);
        // print_r($p_image);die;
        $this->load->view('admin/includes/header', $this->data);
        $this->load->view('admin/product_detail', $this->data);
        $this->load->view('admin/includes/footer');
    }
